...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    ...
    wh = _transpose_and_gather_feat(wh, inds)
    if cat_spec_wh:
        wh = wh.view(batch, K, cat, wh_s)
        clses_ind = clses.view(batch, K, 1, 1).expand(batch, K, 1, wh_s).long()
        wh = wh.gather(2, clses_ind).view(batch, K, wh_s)
    else:
        wh = wh.view(batch, K, wh_s)
    clses = clses.view(batch, K, 1).float()
    scores = scores.view(batch, K, 1)
    if wh_s == 1:
