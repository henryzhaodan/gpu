...
class EstimatorLandmark(nn.Module):

    def __init__(self, config):
        # self.inplanes = 64
        super(EstimatorLandmark, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.config.stage_num >= 2:
            x1 = F.interpolate(x[1], size=(height, width), mode='bilinear', align_corners=False)
            x_cat = torch.cat([x[0], x1], 1)
        if self.config.stage_num >= 3:
            x2 = F.interpolate(x[2], size=(height, width), mode='bilinear', align_corners=False)
            x_cat = torch.cat([x_cat, x2], 1)
        if self.config.stage_num >= 4:
            x3 = F.interpolate(x[3], size=(height, width), mode='bilinear', align_corners=False)
            x_cat = torch.cat([x_cat, x3], 1)
        x = self.head(x_cat)
...
if __name__ == '__main__':
    ...
    config = opts
    model = EstimatorLandmark(config)

    img = np.random.random(size=(1, 256, 64, 64)).astype(np.float32)
