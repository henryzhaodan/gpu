...
def decode_landmarkmap(output, res):
    ...
    for n in range(coords.size(0)):
        for p in range(coords.size(1)):
            ...
            if 1 < px < res[0] and 1 < py < res[1]:
                diff = torch.from_numpy(np.array([hm[py - 1][px] - hm[py - 1][px - 2],
                                                  hm[py][px - 1]-hm[py - 2][px - 1]]))
                coords[n][p] += diff.sign() * .25
    coords += 0.5

    if coords.dim() < 3:
        coords = coords.view(1, coords.size())

    return coords


def decode_transform_landmarkmap(output, center, scale, res):
    coords = get_preds(output)  # float type
